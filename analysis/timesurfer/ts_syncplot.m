...
%   if ischar(parms.refchan) && ~strcmp(parms.refchan,'all')
%     sen = sen(strmatch(parms.refchan,sen));
%   elseif isnumeric(parms.refchan) && ~isempty(parms.refchan)
%     sen = sen(parms.refchan);
%   elseif iscell(parms.refchan) && ~isempty(parms.refchan) && ischar(parms.refchan{1})
%     [s1 s2] = match_str(sen,parms.refchan);
%     sen = sen(s1);
%     clear s1 s2
%   end
