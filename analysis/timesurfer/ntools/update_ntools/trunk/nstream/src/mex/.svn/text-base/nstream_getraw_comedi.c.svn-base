...
/*----------------------------------------------------------------------
%
% Y = NSTREAM_GETRAW_COMEDI(N)
%
% NSTREAM_GETRAW_COMEDI returns N data points back from last written position 
%
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  for(i=0; i < number_of_samples*NSTREAM_NUM_COMEDI_CHANNELS; i++)
  {  
     raw_matrix[NSTREAM_NUM_COMEDI_CHANNELS*number_of_samples - 1 - i] = share->comedi_buffer[write_pos];
     write_pos--; 	 
	 if(write_pos == -1) { 	 
	   write_pos = NSTREAM_COMEDI_NUM_SAMPLES - 1; 
     }
  }  
