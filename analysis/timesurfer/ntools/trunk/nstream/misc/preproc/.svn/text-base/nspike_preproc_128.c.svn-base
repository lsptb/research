...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  memset(raw4,0,sizeof(raw1)); */
      ...
      {
        memcpy(raw1 + (n/chan_count)*60,buf + n,60*SS); // copy 1 sample from 60 channels to first raw buffer
        memcpy(raw2 + (n/chan_count)*60,buf+(n+64*SS),60*SS); // 
        //memcpy(raw3 + (n/chan_count)*60,buf+(n+128*SS),60*SS); //
        //memcpy(raw4 + (n/chan_count)*60,buf+(n+192*SS),60*SS); //
      }
      fwrite(raw1,1,(k/chan_count)*60,raw1_fd);  // write DD samples from first buffer to disk
      fwrite(raw2,1,(k/chan_count)*60,raw2_fd);
      //fwrite(raw3,1,(k/chan_count)*60,raw3_fd);
      //fwrite(raw4,1,(k/chan_count)*60,raw4_fd);
