...
%     clusters_Mmax_all = clusters_Mmax_all(1);
  ...
  clusters_Nmax_pks(1).epochs = clusters_Nmax_pks(1).epochs(~cellfun(@isempty,{clusters_Nmax_pks(1).epochs.R}));
  clusters_Near_pks(1).epochs = clusters_Near_pks(1).epochs(~cellfun(@isempty,{clusters_Near_pks(1).epochs.R}));
  clusters    = clusters_Nmax_pks(1);  % clusters_Cmax_pks(1) % clusters_Mmax_pks(1)
  clustersear = clusters_Near_pks(1);  % clusters_Cear_pks(1) % clusters_Mear_pks(1)
  [alldet.negpeak]    = deal([]);
  [cstdet.negpeak]    = deal([]); tmp={cstdet.pospeak_cluster_number}; [cstdet.cluster_number] = deal(tmp{:});
  [condet.negpeak]    = deal([]); tmp={condet.pospeak_cluster_number}; [condet.cluster_number] = deal(tmp{:});
  det                 = cstdet; % cstdet % matdet
  clusters.epochs     = clusters.epochs(~cellfun(@isempty,{clusters.epochs.R}));
  clustersear.epochs  = clustersear.epochs(~cellfun(@isempty,{clustersear.epochs.R}));
  
  % IDI
  Nidi        = diff([clusters_Nmax_pks(1).epochs.RefTime]);
  Nidi_mu(s)  = mean(Nidi);
  Nidi_std(s) = std(Nidi);
  Nidi_min(s) = min(Nidi);
  Nidi_max(s) = max(Nidi);
  ...
  Ncstsiz     = cellfun(@length,{clusters_Nmax_pks(1).epochs.InvolvedChans});
  ...
  vardelay    = arrayfun(@(x)var(x.Delays),clusters_Nmax_pks(1).epochs);
...
%   1,toc
  ...
  T     = 0;
  nn    = 0;
  tref  = [clusters_Nmax_pks(1).epochs.RefTime];
...
%   2,toc
  ...
  detcnt(s)  = round(length(clusters_Nmax_pks(1).epochs)/2); 
  ...
  Rvals     = [clusters.epochs.R];%arrayfun(@(x)max(x.R),clusters_Cmax_pks(1).epochs);
  pvals     = [clusters.epochs.p];%arrayfun(@(x)max(x.p),clusters_Cmax_pks(1).epochs);
  Nvals     = [clusters.epochs.N];%arrayfun(@(x)max(x.N),clusters_Cmax_pks(1).epochs);
%   3,toc
...
% % cluster size
% Ccstsiz   = cellfun(@length,{clusters_Cmax_pks(1).epochs.InvolvedChans});
% Ncstsiz   = cellfun(@length,{clusters_Nmax_pks(1).epochs.InvolvedChans});
