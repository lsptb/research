...
load s1/matfiles/SO_clusters_noflip.mat
params = SO_params(1);

% dat = ts_preproc(epoch_data,'bpfilter','yes','bpfreq',[.1 4]);
cst = clusters_Near_all(1).epochs;
cst = cst([cst.p] < .05 & [cst.R] > .5);
T   = [cst.HistTime];
t   = clusters_Near_all(1).abs_toilim(1):1/clusters_Near_all(1).sfreq:clusters_Near_all(1).abs_toilim(2);
tt  = dat.epochs.time;
Fs  = dat.sfreq;
Nch = dat.num_sensors;
pad = round(.5*Fs); % 500ms
...
for k = 1:length(T)
  ...
  ik  = nearest(tt,tk);
  sel = ik + (-pad:pad);
  tmp = dat.epochs.data(:,sel);
  imagesc(tt(sel),1:Nch,tmp);
  title(sprintf('%gsec: %02i(%02i,%g)',tk,cst(k).R,cst(k).p,cst(k).N));
  xlabel('time (sec)'); ylabel('channel number');
  vline(tk,'k'); vline(tk-.2,'r'); vline(tk+.2,'r');
  pause
end
  
%   tmp = mat2cell(tmp,ones(1,nchan),size(tmp,2));
%   big = cellfun(@max,tmp);
%   tmp = cellfun(@(x,y)(x/y),tmp,num2cell(big),'uniformoutput',false);
%   tmp = cell2mat(tmp);
