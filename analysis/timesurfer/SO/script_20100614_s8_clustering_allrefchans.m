  ...
  %         telapsed = toc(tstart); fprintf(fid,'Time elapsed: %g min\n',telapsed/60);
    ...
    %       c  = 1;     % pktype
      ...
      %       pause
          ...
          fprintf(fid,'For [%s] %s clusters (n=%g of %g) with R > %g & nchan > %g:\n',reflabel,clusters(c).peaktype,length(pk),length(clusters(c).epochs),Rth,Nth);
