...
% 3rd party revision on 2009/05/21 by jsherfey @ UCSD's MMIL
% skip events table if nevents is not an integer
%
% $Log: loadcnt.m,v $
...
if type == 'cnt' 
      ...
      elseif eT.teeg==1
          ...
          %% inint(nevents) added by JSS on 21-May-2009
          if nevents > 0 && isint(nevents)
              ev2(nevents).stimtype  = [];
              for i=1:nevents
                  ev2(i).stimtype      = fread(fid,1,'ushort');
                  ev2(i).keyboard      = fread(fid,1,'char');
