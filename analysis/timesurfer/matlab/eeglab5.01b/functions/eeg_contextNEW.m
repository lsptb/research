...
% 8/04/05 added a test to accommodate cell2mat change in EEG.epoch structure (~line 316) -jo
...
%%%%%%%%%%%%%%% cycle through target types %%%%%%%%%%%%%%%%%%
    ...
    while ~istarget & tidx<=length(targets)           % for each potential target type
        if (strcmpi(num2str(EEG.urevent(uridx).type),targets(tidx)))        % if is a target type
                ...
                for z = 1:length(EEG.event(evidx).epoch) % for each epoch the event is in
                    ...
                    for e = 1:length(EEG.epoch(ep).event) % for each event in the epoch
                        if EEG.epoch(ep).event(e) == evidx            %%  *****************
                            if length(EEG.epoch(ep).eventlatency) > 1 %%  This is the check
                                evlt = EEG.epoch(ep).eventlatency{e}; %% for new version of 
                            else                                      %% EEG.epoch structure  
                                evlt = EEG.epoch(ep).eventlatency{1}(e);%%  *****************
                            end;
                            if evlt == 0
    ...
    if istarget  % if current event is a target type
        ...
        if ~isempty(negpos)
            ...
            while uidx > 0 & npidx <= nnegpos                 % search through previous urevents
                if strcmpi(num2str(EEG.urevent(uridx).type),'boundary')  % flag boundary urevents
