...
% signal = single(zeros( chan, pnts*nsweeps));
...
% 						signal(:,[((count_selected-1)*pnts+1):count_selected*pnts]) = single(buf);
...
% signal = single(signal(chanlist, 1:(count_selected-1)*pnts));
