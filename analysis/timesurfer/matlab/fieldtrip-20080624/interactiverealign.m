...
% cfg.target.elec
% cfg.target.grad
% cfg.target.headshape
% cfg.target.headshapestyle  = 'vertex' (default), 'surface' or 'both'
%
% cfg.template.vol
% cfg.template.elec
% cfg.template.grad
...
if ~isfield(cfg, 'target') || ~isfield(cfg, 'template')
  error('target and template fields are required');
end

if ~isfield(cfg.target, 'vol'),              cfg.target.vol = [];                   end
if ~isfield(cfg.target, 'elec'),             cfg.target.elec = [];                  end
if ~isfield(cfg.target, 'grad'),             cfg.target.grad = [];                  end
if ~isfield(cfg.target, 'headshape'),        cfg.target.headshape = [];             end
if ~isfield(cfg.target, 'headshapestyle'),   cfg.target.headshapestyle = 'vertex';  end
...
setappdata(fig, 'target', cfg.target);
setappdata(fig, 'template', cfg.template);
...
cfg.version.id = '$Id: interactiverealign.m,v 1.1 2008/06/24 13:51:15 roboos Exp $';
...
if ~isempty(template.vol)
  ...
  for i = 1:numel(vol.bnd)
    hs = triplot(vol.bnd(i).pnt, vol.bnd(i).tri, [], 'edges');
...
if ~isempty(template.elec)
  elec = template.elec;
  hs = triplot(elec.pnt, [], [], 'nodes');
  try, set(hs, 'EdgeColor', 'b'); end
  if isfield(elec, 'line')
    hs = triplot(elec.pnt, elec.line, [], 'edges');
    try, set(hs, 'EdgeColor', 'b'); end
  end
end
...
if ~isempty(template.headshape)
  ...
  if isfield(headshape, 'pnt') && ~isempty(headshape.pnt)
    if strcmp(template.headshapestyle, 'surface') || ...
        strcmp(template.headshapestyle, 'both')
      if ~isfield(headshape, 'tri') || isempty(headshape.tri)
        headshape.tri = projecttri(headshape.pnt);
      end
      triplot(headshape.pnt, headshape.tri,  [], 'faces_blue');
      alpha(str2num(get(findobj(fig, 'tag', 'alpha'), 'string')));
    end

    if strcmp(template.headshapestyle, 'vertex') || ...
        strcmp(template.headshapestyle, 'both')
      hs = triplot(headshape.pnt, [], [], 'nodes');
      set(hs, 'Color', 'b');
    end
  end

  if isfield(headshape, 'fid') && ~isempty(headshape.fid.pnt)
    triplot(headshape.fid.pnt, [], [], 'nodes_blue');
  end
end

if ~isempty(target.vol)
  vol = transform_vol(H, target.vol);
  for i = 1:numel(vol.bnd)
    hs = triplot(vol.bnd(i).pnt, vol.bnd(i).tri, [], 'edges');
...
if ~isempty(target.grad)
  grad = transform_sens(H, target.grad);
  hs = triplot(grad.pnt, [], [], 'nodes');
  set(hs, 'Color', 'r');
end

if ~isempty(target.headshape)
  headshape = transform_headshape(H, target.headshape);

  if isfield(headshape, 'pnt') && ~isempty(headshape.pnt)
    if strcmp(target.headshapestyle, 'surface') || ...
        strcmp(target.headshapestyle, 'both')
      if ~isfield(headshape, 'tri') || isempty(headshape.tri)
        headshape.tri = projecttri(headshape.pnt);
      end
      triplot(headshape.pnt, headshape.tri,  [], 'faces_red');
      alpha(str2num(get(findobj(fig, 'tag', 'alpha'), 'string')));
    end

    if strcmp(target.headshapestyle, 'vertex') || ...
        strcmp(target.headshapestyle, 'both')
      hs = triplot(headshape.pnt, [], [], 'nodes');
      set(hs, 'Color', 'r');
    end
  ...
  if isfield(headshape, 'fid') && ~isempty(headshape.fid.pnt)
    triplot(headshape.fid.pnt, [], [], 'nodes_red');
  end
end
