...
if ~isfield(surf,'coords') | ~isfield(surf, 'faces')
  fprintf('%s: ERROR: input surf must contain coords and faces\n',mfilename);
  return;
end;
...
for f=1:surf.nfaces
  v = surf.faces(f,:);
  v_coords = surf.coords(v,:);
  edges = [v_coords(2,:)-v_coords(1,:);...
           v_coords(3,:)-v_coords(1,:);...
           v_coords(3,:)-v_coords(2,:)];
