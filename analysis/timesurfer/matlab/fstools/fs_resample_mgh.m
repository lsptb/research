function varargout = fs_resample_mgh(mghfile, varargin)
%varargout fs_resample_mgh(mghfile, varargin)
%
% Purpose:
%   This function does simple "resampling" on an mgh (timeseries),
...
%  Last Modified by:  Ben Cipollini  08/10/2007
  ...
  parms           = mmil_args2parms( varargin, ...
                                     { 'window_size',[],       [0:100],...
                                       'frames_out',     [],       [1:10000],...
                                       'algorithm',   '',         {'linear',...
                                       'average', 'max', 'first','mid','last'}, ...
                                       'tfirst',      0,          [0 Inf], ...
                                       'tlast',       Inf,        [0 Inf], ...
                                       ... %I/O args
                                       'outfile',     [],    [],...
                                       ... %user prefs
                                       'force',        false,      sort([false true]),... 
  ...
  % get the mgh
  mmil_logstr(parms, 'Loading mgh file...');
  ...
  if (parms.tfirst < 0) 
    mmil_error(parms, 'tfirst must be >=0');
  
  elseif (parms.tlast > volsz(4)-1)
    mmil_error(parms, 'tlast must be <= the last frame index (%d)', volsz(4)-1);
  ...
  if (~parms.force && exist(parms.outfile,'file'))
    mmil_logstr(parms, 'NOTICE: not recomputing resampling: output file exists @ %s', parms.outfile);
    return;
  ...
  elseif (~ismember(parms.algorithm, {'linear', 'zod'}))
    ...
    % resample ("time" is 0-based, so we have to subtract 1 from MATLAB
    % indices)
    ts = resample(ts, 0:parms.window_size:parms.window_size*(volsz(4)-1), parms.algorithm);
  
    % switch our time back to the last dimension to get mgh format.
    new_vol = permute(ts.data, ts_dim);

  end;
  
  % save out the mgh
  mmil_logstr(parms, 'Saving to mgh @ %s', parms.outfile);
