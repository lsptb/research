...
% Last Mod: 06/26/09 by Don Hagler
...
X = zeros(nrows,ncols);
X(:,end) = ones(nrows,1); % baseline / intercept
j = 1;
if nconds > 1
  for s=1:nsubs
    for i=1:nconds
      if i>1, X(j,i-1) = 1; end;
      if nregs
        X(j,nconds-1+[1:nregs]) = parms.regressors(s,:);
      end;
      j = j + 1;
    end;
  end;
...
tic;
results = calc_voxelwise_glm(X,datamat,...
  parms.contrast_vectors,parms.contrast_names,...
  nconds,parms.groupregs);
...
function results = calc_voxelwise_glm(X,datamat,cont_vecs,cont_names,...
  ...
  %% todo: to collapse across runs, use covariance matrix with off-diags?
  C = Xi*Xi'; % what is C?
  ...
  results.dof = results.nvols - results.nparams;
  results.beta_cov = C;
  results.X = X;
  results.Xi = Xi;
  ...
  for i=1:length(cont_vecs)
    ...
    if any(ind_nonzero<nconds)
      tmp_nconds = 1;
    else
      % don't count the extra conditions as free parameters if we are collapsing across
      %  shouldn't be penalized for averaging
      tmp_nparams = results.nparams - (nconds-1);
    end;
    results.contrasts(i).n = results.nvols/tmp_nconds;
    
    results.contrasts(i).dof = results.contrasts(i).n-tmp_nparams;
    ...
    results.contrasts(i).pvals = (results.contrasts(i).tstats<0).*...
                                 log10(2*tcdf(results.contrasts(i).tstats,...
