...
% mmil_barh - creates horizontal bar plot with optional labels and error bars
%
% Usage:
...
%    {default: 1}
%  'text_v_offset' - vertical offset for data name locations
%    {default: 0}
%  'text_v_scale' - vertical scaling for data name locations
%    {default: 1}
%  'title' - plot title
%    {default: []}
%  'xlabel' - x axis label
...
%  last modified: 01/16/07   by Don Hagler
%
...
clf;
barh(data,parms.barwidth);
h_ax_data = gca;
axis ij;
set(h_ax_data,'Color','none')
set(h_ax_data,'FontSize',parms.fontsize);
...
if ~isempty(parms.group_names)
  legend(parms.group_names,'Location','EastOutside');
end;
...
if ~isempty(parms.data_names)
  ...
  for i=1:num_meas
    if num_meas>1
      y = parms.text_v_offset + pos(2) +...
          ((i-1)/(num_meas-1))*pos(4)*parms.text_v_scale;
    else
      y = parms.text_v_offset + pos(2);
    end;
    h_text(i) = text(x,y,parms.data_names{i},...
      'HorizontalAlignment','right','FontSize',parms.fontsize);
    tmp = get(h_text(i));
