function mmil_makeBEMsurfs_fromT1(fname_in,outdir,forceflag)
%function mmil_makeBEMsurfs_fromT1(fname_in,outdir,forceflag)
...
fprintf(fid,'  %s -surf bem -useSRAS "$indir/%s" "$outdir2/%s"\n',...
  bemsurfs_cmd,fname_in,fname_brain);
fprintf(fid,'endif\n');
fprintf(fid,'if ((! -e $fname_surf1) || (! -e $fname_surf2) || (! -e $fname_surf3) || $forceflag) then\n');
fprintf(fid,'  exit\n');
fprintf(fid,'endif\n');
...
fprintf(fid,'  %s "fs_convert_fsurf2tri(''$fname_surf1'',''$fname_tri1'',$nverts); fs_convert_fsurf2tri(''$fname_surf2'',''$fname_tri2'',$nverts); fs_convert_fsurf2tri(''$fname_surf3'',''$fname_tri3'',$nverts); exit;"\n',...
  matlab_cmd);
fprintf(fid,'endif\n');
fprintf(fid,'cd $cwd\n');
fclose(fid);
