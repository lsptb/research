...
% ts_ezplot(data,'key','value',...)
% ts_ezplot('key','value',...)
% default: multiplot and topoplot sequence for each condition
%
...
% layout / layoutfile
%
% reject_data / rejectfile
% badchans / badchanfile
% 
%
% ts_ezplot('event',[1:8],'format','jpg','title','NY70-FWIO')

% in 3 plots overlay events 1/2/3, 3/5, 1/3/5
% events = {[1 2 3] [3 5] [1 3 5]}
% in 3 plots show events 1/2/3
% events = [1 2 3] & overlay=0 or events = {[1] [2] [3]}
%
%
% structure of main loop:
% loop over channel types
  % loop over condition vectors in the condition list 
    % [condition labels]
    % loop over conditions in the condition vector [zscore,conversion,mask]
    % [layout]
    % loop over layouts (pages) required to plot this condition vector
      % [cfg]
...
ieegflag = (strcmpi(parms.chantype,'ieeg') || (ismember('eeg',unique({data.sensor_info.typestring})) ...
           && isempty(intersect({'grad1','grad2','mag','meg','other'},{data.sensor_info.typestring}))));
...
if ~parms.trials_flag && (strcmp(datatype,'epoch_data') || ...
  ...
  mmil_logstr(parms,'%s: averaging data over epochs',mfilename); 
%  fprintf('%s: averaging data over epochs\n',mfilename);
  data = ts_trials2avg(data,varargin{:});
  datafield = 'averages';
  parms.datafield = 'averages';
end
% now: data is either [ERP] = chan x time, or [power] = chan x time x freq

if parms.plotAvg && ~powflag
  if isempty(parms.datafile)
    error('%s: ''plotAvg'' requires the full filename of an epoch_data or avg_data structure.\nNote: pass the data structure using ''multiplot'' or the filename using ''plotAvg.''',mfilename);
  end
...
%     title          = parms.title;          
    ...
    dat = ts_data_selection(dat,'chantype',parms.chantype,'removebadchans',parms.removebadchans); % added 3/26/2010
...
%%    
    for c = 1:length(conds)
      if parms.freqcorr ~= 0 && powflag
        mmil_logstr(parms,'%s: correcting for 1/f^%g scaling of power spectrum',mfilename,parms.freqcorr_alpha);
%         fprintf('%s: correcting for 1/f^%g scaling of power spectrum\n',mfilename,parms.freqcorr_alpha);
    ...
    for p = 1:npages
      ...
      if parms.ezmultiplot
        ...
        if ~err
          ...
          mmil_logstr(parms,'%s: an ezmultiplot error occurred while processing page %g',mfilename,p);
%           fprintf('%s: an ezmultiplot error occurred while processing page %g\n',mfilename,p);
        end
...
%         figure; 
      ...
      if parms.topoplot
        try 
          ...
          mmil_logstr(parms,'%s: topoplot failed!',mfilename);
%           fprintf('%s: topoplot failed!\n',mfilename);
          close; %continue;
        end
      end
      if parms.multiplot
...
%         end
      ...
      if parms.singleplot
        try
          ...
          mmil_logstr(parms,'%s: singleplot failed!',mfilename);          
%           fprintf('%s: singleplot failed!\n',mfilename);
          close; %continue;
        end
      end        
      if parms.close
...
if strcmp(parms.zlim,'maxmin') || strcmp(parms.zlim,'absmax') || strcmp(parms.zlim,'symmetric')
  ...
  if cfg.get_zlims
      zlims = cfg.zlim;
      mmil_logstr(parms,'%s: %s zlimits = [%g %g]',mfilename,parms.zlim,zlims(1),zlims(2));
%       fprintf('%s: %s zlimits = [%g %g]\n',mfilename,parms.zlim,zlims(1),zlims(2));
    return;
  end
end
...
if ~isempty(parms.statfile)
  mmil_logstr(parms,'%s: loading statistics file: %s',mfilename,parms.statfile);
%   fprintf('%s: loading statistics file: %s\n',mfilename,parms.statfile);
  load(parms.statfile);
...
if exist(outfile,'file') && ~parms.overwrite
  mmil_logstr(parms,'%s: not saving figure.  File already exists: %s',mfilename,outfile);
%   fprintf('%s: not saving figure.  File already exists: %s\n',mfilename,outfile);
  return;
end
...
try 
  mmil_logstr(parms,'%s: Saving figure: %s',mfilename,outfile);
%   fprintf('%s: Saving figure: %s\n',mfilename,outfile);
%   print(gcf,printcmd{:},outfile);
  print(gcf,printcmd{:},outfile);
catch
  mmil_logstr(parms,'%s: failed to save figure: %s',mfilename,outfile);
%   fprintf('%s: failed to save figure: %s\n',mfilename,outfile);
end
