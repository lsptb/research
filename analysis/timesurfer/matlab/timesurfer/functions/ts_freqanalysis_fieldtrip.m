...
%
% Created by: ?? 					- Rajan
% Modified:		13-Sep-2008 - Jason Sherfey 
%
...
data_in  = ts_data_selection(data_in,'channels',parms.channels,'chantype',parms.chantype,'events',parms.event_codes); 

if ~isempty(parms.filename)
  [pathstr,name,ext,versn] = fileparts(parms.filename{1});
    ...
    if rejparms.reject_flag
        ...
        if parms.save_flag && parms.trials_flag && ~(isfield(faparms,'output') && strcmp(faparms.output,'powandcsd'))
          ...
          if parms.trials_flag && ~parms.save_flag
            ...
            ft_timefreq         = freqanalysis(faparms,ft_timelock,1);		
            clear ft_timelock
            epoch_template.sensor_info = data_in.sensor_info;
            epoch_template.num_sensors = length(epoch_template.sensor_info);
            data_out(i) = ts_fieldtrip2timefreq(ft_timefreq,epoch_template);%data_in);            
          else
            faparms.channel = 'all';
...
if isfield(parms,'filename') && ~isempty(parms.filename)
  ...
  [dum, file_tag, ext, versn] = fileparts(parms.f_name{1});
