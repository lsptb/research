...
if isfield(data.epochs,'trial_info') && ~isempty([badtrl{:}])
  for k = 1:length(data.epochs)
    ind = find([data.epochs(k).trial_info.epochnum]);
    [data.epochs(k).trial_info(ind(badtrl{k})).badtrial] = deal(1);
    ind = ~[data.epochs(k).trial_info.badtrial];
    tmp = num2cell(1:sum(ind));
    [data.epochs(k).trial_info(ind).epochnum] = deal(tmp{:});
  end
end
