...
%
% Created By:       Ben Cipollini on 06/01/2007
% Last Modified By: Don Hagler    on 08/21/2007
% Last Modified By: Don Hagler    on 12/02/2008
%

%% todo:
%    test for polar angle retinotopy data (polar flag)
%    use max frame num for zero padding (not tlast)
%%
...
%                  {default: []} (all)
  ...
  parms           = mmil_args2parms( varargin, {...
    'nocache'   ,     false, sort([true false]),...
    'force',          false, sort([true false]),...
    'hashnames',      true,  sort([true false]),...
    ...
    'gravity',        'NorthEast',[],...
    ...
    'fgcolor',        '#ffffff',    [],... %unpublished feature
    'transparent',    '#000000',    [],... %unpublished feature
    'delay',          2,            [1 Inf], ...
    ...
    ...
    'tlast',          Inf,        [0 Inf], ...
    'tstep',          1,          [1 Inf], ... %
    'step_type',      'average',  [],... % leave validation up to fs_resample_mgh
    'position',       [],         [] ...
    'scalebar',       false, sort([true false]),...
    'colscalebar',    false, sort([true false]),...
    'polar',          false, sort([true false]),...
    'checksum'        '',         [], ... %unpublished feature
    ...
    'verbose',        true,       sort([true,false]),...
    'logfile',        [],         [], ...
    'logfid',         1,          [] ...
  });
									
%  parms = mergeParms( parms, getMontageParms(parms.montage) );
  ...
  logstr(parms,'Checking dependencies...');
  ...
  %surmfgh      
  elseif (~smartunix('which fs_surfmgh.csh', 'result'))
  ...
  logstr(parms,'Setting up image properties...');
  ...
  for i=1:length(parms.srcfile)
    ...
    if (~exist(parms.srcfile{i}, 'file'))
  	...
  	setenv('SUBJECTS_DIR', parms.SUBJECTS_DIR{i}); %% needed by fs_resample_surf?
	
	% Convert all input files to mgh
  	if (strcmp(fileparts2(parms.srcfile{i},'ext'),'.stc'))
	  % convert to mgh!
	  mghfile	= regexrep(parms.srcfile{i}, '\.stc$', '.mgh');
      ts_stc2mgh(parms.srcfile{i}, mghfile);
	  parms.srcfile{i} = mghfile;
	  %[v,sr,st,sol]	= ts_read_stc(parms.srcfile{i});
	  %nFrames = size(sol,2);
  	  %clear('v','sr','st','sol');
	  %mgh file
	end;
	...
	if (parms.tstep ~= 1)
	  ...
	  fs_resample_surf(parms.srcfile{i}, parms.tstep, resampled_mghfile);
  ...
  for i=1:num_images
    logstr(parms, 'Rendering images for surface [%d] of [%d], frames [%d] - [%d]',...
                   i, num_images, parms.tfirst{i}, parms.tlast{i});
    
    if (isempty(parms.checksum{i}))
      parms.checksum{i} = fileparts2(parms.srcfile{i},'name');
      if parms.hashnames
        parms.checksum{i} = sprintf('%s-hash%d',...
                                    parms.checksum{i},...
                                    bogushash(fileparts2(parms.srcfile{i},'path')));
      end;      
...
%                 ' -silent'];
    [r,o]=smartunix(cmd);
    if (~r)
      error('%s: Failed to run fs_surfmgh: %s\n\n%s', mfilename, o, cmd);
    end;
  end; %for each image
...
%        rgbdir = fullfile(parms.outdir, 'rgb');
        image_file = sprintf('%s/%s-%s-%s-%s-%s.rgb',...
                             rgbdir,...
                             parms.image_stem{j}, ...
                             padstr(t,1+floor(log10(parms.tlast{j}))), ...
                             parms.hemi{j}, parms.surface{j}, parms.view{j});
        if parms.trim
          fname = sprintf('%s/%s-trim.rgb',rgbdir,fileparts2(image_file,'name'));
          if ~exist(fname,'file') || parms.force
            cmd = sprintf('%s/convert -trim %s %s', ...
                          parms.IMAGEMAGICK_DIR, image_file,fname);
            [status,result] = smartunix(cmd);
            if ~status % this is opposite of behavior of unix function! DH
              disp(cmd);
       ...
       for im=1:length(parms.position)
        ...
        if (parms.position{im} == j)
          if (~exist(image_files{im},'file'))
            error('%s: can''t find surface image (error running fs_surfmgh?): %s\n', ...
      ...
      % Annotate the montage
        ...
        cmd = sprintf(['%s %s -undercolor \\%s -fill \\%s -pointsize %d -gravity %s -draw "text %d,%d ''' parms.timestamp_format '''" %s'], ...
                       fullfile(parms.IMAGEMAGICK_DIR,'convert'), ...
                       montaged_files{end},...
  ...
  logstr(parms, 'Concatenating images to create mpg...'); 

  mpg_file  = fullfile(parms.outdir, [parms.outstem '.mpg']);
  if (exist(mpg_file, 'file'))
    logstr(parms, 'NOTICE: overwriting existing movie: %s', mpg_file);
    delete(mpg_file);
  end;
  ...
  if (~status)
    if (exist(mpg_file,'file'))
      logstr(parms, 'WARNING: movie was created, but with warnings:\n\n%s',result);
	  else
      disp(cmd);
  ...
  if ( parms.nocache )
    logstr(parms, 'Removing intermediate cache files');
    rmdir(fullfile(parms.outdir,'rgb'), 's');
    rmdir(fullfile(parms.outdir,'montages'), 's');
  end;
  
  % ALWAYS clean up after fs_surfmgh
  ...
  %
  %
    [r, mf] = unix('which montage');
    d = fileparts2(mf, 'path');
    if (r || ~exist(fullfile(d, 'convert'), 'file'))
