...
%     if strcmp(who('-file',thisfile,'epoch_data'),'epoch_data')
        ...
        case '.avg'
          ...
          clear tmp
      case '.edf'
        data_arr(f)  = ts_loadedf('datafile',thisfile,...
                                  'evntfile',parms.evntfile,...
                                  'channels',parms.channels,...
                                  'stim_delay',parms.stim_delay,...
                                  'prestim' ,-parms.timelimits(1),...
                                  'poststim',parms.timelimits(2),...
                                  'trigchan',parms.trigchan,...
                                  'trigthresh',parms.trigthresh,...
                                  'trig_minduration',parms.trig_minduration,...
                                  'trig_minseparation',parms.trig_minseparation,...
                                  'continuous',parms.continuous,...
                                  'oldchanlabels',parms.oldchanlabels,...
                                  'newchanlabels',parms.newchanlabels,...
                                  'valid_event_codes',parms.events);
        if f == length(datafile), parms.stim_delay = []; end
    end
...
%       fprintf('%s: warning: skipping combinations. # average combos does not equal # codes.\n',mfilename);
    ...
    if isempty(parms.filename)
      filename = sprintf('%s/%s_averages_events%s.mat',parms.rootoutdir,parms.prefix,strrep(num2str([avg_data.averages.event_code]),'  ','_'));
%       filename = sprintf('%s/%s_averages_cond%g.mat',parms.rootoutdir,parms.prefix,c);
    else
...
%           filename = sprintf('%s/%s_epochs%s_cond%02i.mat',parms.rootoutdir,parms.prefix,tag,epoch_data.epochs.event_code);
          filename = sprintf('%s/%s_epochs%s_event%02i.mat',parms.rootoutdir,parms.prefix,tag,epoch_data.epochs.event_code);
        else
          if ~iscell(parms.filename), parms.filename = {parms.filename}; end
          [fpath fname] = fileparts(parms.filename{1});
