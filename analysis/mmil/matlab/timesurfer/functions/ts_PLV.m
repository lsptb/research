...
if parms.dsfact ~= 1 && ~isempty(parms.dsfact)
  if ~isempty(parms.toilim)
    timefreq_data = ts_data_selection(timefreq_data,'toilim',parms.toilim,'verbose',parms.verbose);
  end    
...
for e = 1:length(parms.events)
  ...
  for k=1:numcmb
    i = cmb(k,1);
    j = cmb(k,2);
    mmil_logstr(parms,'calculating %s matrix %i of %i (%s,%s) for event(s): [%s] (%gs elapsed)',[parms.output{:}],k,numcmb,sens(i).label,sens(j).label,num2str(events),toc);    
    if parms.verbose
    ...
    if parms.loadflag
      ...
      data = ts_data_selection(timefreq_data,'channels',[i j],'removebadchans',1,'events',events,'verbose',parms.verbose);
      if length(events) > 1
