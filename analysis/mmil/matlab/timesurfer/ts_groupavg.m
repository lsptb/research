...
%  Last Mod: 10/21/2008 by Don Hagler
%

%% todo: log file?
%% todo: averages for frames from f1 to f2
%% todo: use new fs_groupavg
  ...
  for g=1:ngroups
    ...
    if ~exist(outname_mean,'file') | ~exist(outname_stdv,'file') |...
      ...
      for f=1:nframes
        ...
        results = fs_groupavg_old(studyinfo.datafiles(s_ind,c),...
          groupname,[],f,parms.offset,parms.verbose);
        ...
        outvol_tstat(:,:,:,f) = results.groups.tstat;
        outvol_pval(:,:,:,f) = results.groups.sig;
      end;
      n = results.groups.n;
      ...
      fs_save_masked_mgh(outvol_mean,outname_mean,...
        hemi,parms);
      fs_save_masked_mgh(outvol_stdv,outname_stdv,...
        hemi,parms);
      fs_save_masked_mgh(outvol_tstat,outname_tstat,...
        hemi,parms);
      fs_save_masked_mgh(outvol_pval,outname_pval,...
        hemi,parms);
      save(outname_n,'n','df');
    end;
  end;
    ...
    for g=1:ngroups
      ...
      if ~exist(outname_mean,'file') | ~exist(outname_stdv,'file') |...
        ...
        for f=1:nframes
          ...
          results = fs_groupavg_old(studyinfo.datafiles(s_ind,[cA_ind,cB_ind]),...
            groupname,[1 -1],f,parms.offset,parms.verbose);
          if isempty(results)
...
function fs_save_masked_mgh(vol,fname,hemi,parms)
  if parms.mbmask_flag
