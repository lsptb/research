...
for trial = 1:length(data.trial)
    endd              = data.time{trial}(length(data.time{trial}));
    begin             = data.time{trial}(2);
    ...
    pre_coef          = data.trial{trial}(1);
    pst_coef          = data.trial{trial}(length(data.trial{trial}));
    pre_adjusted      =-(pre  - pre_coef);
    pst_adjusted      =-(post - pst_coef);  
    % part A
    D2                = diff(diff([pre data.trial{trial}(1:length(pre))]));
    if max(D2) < 5*mean(D2)
        temp = [pre data.trial{trial}];
    else
        temp = [-(pre - pre_coef) (data.trial{trial} - pre_coef)];
        temp =   temp + pre_coef;
    end
    I1 = 1;
    I2 = length(pre_adjusted);
    pre_part = data.trial{trial}(I1:I2);
    if max(temp(1:length(pre))) - min(pre_part) > 1.1*(max(pre_part) - min(pre_part)), 
        temp(I1:I2) = -pre; 
    end
    data.trial{trial} = temp;
    % part B
    D2 = diff(diff([data.trial{trial} post]));
    if max(D2) < 5*mean(D2)
        temp = [data.trial{trial} post];
    else
        temp = [(data.trial{trial} - pst_coef) -(post - pst_coef)];
        temp =   temp + pst_coef;
    end
    I1 = length(data.trial{trial})-length(pre)+1;
    I2 = length(data.trial{trial});
    pst_part = data.trial{trial}(I1:I2);
    if max(temp(length(data.trial{trial}):length(temp))) - min(pst_part) > 1.1*(max(pst_part) - min(pst_part)), 
        temp(length(temp)-length(pre)+1:length(temp)) = -pre; 
    end
    data.trial{trial} = temp;
...
cfg_c = cfg; dta = data; orig_foi = cfg.foi; ndx = 1; clear freq freq_;
% if length(cfg.foi) > 1, 
