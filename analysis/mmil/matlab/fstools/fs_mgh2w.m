...
% last modified:  10/03/08 Don Hagler
%
...
if ny~=1 | nz~=1
  error(sprintf('mgh file is not a surface file -- size is [%d,%d,%d,%d]',...
...
if isempty(vol)
  error(sprintf('failed to read mghfile %s',mghfile));
end;
