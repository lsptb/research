...
while interactive
  ...
  if sum(~trlsel) > 0,  fprintf('not showing trials: %s\n',num2str(find(trlsel==0)));    end
  if sum(~chansel) > 0
    str = '';
    ix  = find(chansel==0);
    lb  = label(chansel==0);
%     str = num2str(find(chansel==0));
    for i = 1:length(lb)
      str = [str num2str(ix(i)) ' (' lb{i} ')  '];
    end
    fprintf('not showing channels: %s\n',str); 
  end
