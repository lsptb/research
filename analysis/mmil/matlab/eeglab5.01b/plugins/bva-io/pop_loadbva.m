...
function [EEG, com] =  pop_loadbva(filename)
    ...
    for index = 1:length(bva.Markers)
        EEG.event(index).type        = bva.Markers(index).Type;
        EEG.event(index).latency     = bva.Markers(index).Position;
        EEG.event(index).points      = bva.Markers(index).Points;
        try
            EEG.event(index).description = bva.Markers(index).Description;
        catch, end;
        try
            EEG.event(index).chan        = bva.Markers(index).Chan;
        catch, end;
        try
            EEG.event(index).channelnumber = bva.Markers(index).ChannelNumber;
        catch, end;
        if bva.SegmentCount > 1
            EEG.event(index).epoch       = index;
            EEG.event(index).latency     = bva.Markers(index).Position+(index-1)*EEG.pnts;
        else
            EEG.event(index).latency     = bva.Markers(index).Position;
        end;
    end;
