% ntools_proc_decimate(recording_filename_root)
...
function ntools_proc_decimate(recording_filename_root)

    recording_filename_root = make_filename_root(recording_filename_root); %removes suffix if necess
    [sfreq, num_channels, num_samples, precision, chan_names] = ntools_hdrxml_read(recording_filename_root);
