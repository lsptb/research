...
/*----------------------------------------------------------------------
%
% Y = NSTREAM_GETMUINT_NSPIKE_16(T1,T2)
%
% NSTREAM_GETMUINT_NSPIKE_16 returns data for a given time interval from the shared memory buffer
%
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (share->nspike_time < to_samp)
      mexErrMsgTxt("T2 > nspike_time");

  if (from_samp < share->nspike_time - (NSTREAM_DURATION * NSTREAM_SR)) 
  ...
  plhs[0] = mxCreateDoubleMatrix(16, dur_samp, mxREAL);
...
/*  write_pos--; */
  ...
  {  
    for(ich = 0; ich < 16; ich++)
      {
       return_matrix[16*(dur_samp-1) - output_pos + ich ] = share->nspike_buffer[write_pos+ch[ich]];
      }
    output_pos+=16;
    write_pos -= NSTREAM_NUM_NSPIKE_CHANNELS;
    if(write_pos < 0) {
