...
/*----------------------------------------------------------------------
%
% Y = NSTREAM_GETRAW_COMEDI_ALL()
%
% NSTREAM_GETRAW returns all data from comedi shm buffer 
%
% Inputs:	 None 
%
% Outputs:  Y  =  [M,N] Matrix, buffer data
%
*/
/*----------------------------------------------------------------------*/
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  plhs[0] = mxCreateDoubleMatrix(NSTREAM_NUM_COMEDI_CHANNELS,NSTREAM_DURATION * NSTREAM_SR, mxREAL);
  raw_matrix = mxGetPr(plhs[0]); 


  for(i=0; i < NSTREAM_COMEDI_NUM_SAMPLES; i++)
  {  
     raw_matrix[i] = share->comedi_buffer[i];
  }  
