...
%             [tmpPSD{1:nchan}]      = deal([]);            
            ...
            if ischar(parms.PSD_offset) && strcmp(parms.PSD_offset,'epochpad')
              PSD_offset = epochpad; 
            elseif isnumeric(parms.PSD_offset) && ~isempty(parms.PSD_offset)
              PSD_offset = ceil(parms.PSD_offset*Fs/1000); % convert ms to indices
            else
              PSD_offset=0;
            end 
            psdtix   = PSD_offset+epochtix(1:round(length(epochtix)*parms.PSD_fraction));      
            if parms.Coh_flag
              if strcmp(parms.Coh_time,'epoch')
                cohtix   = refpeak + [-clusterpad:clusterpad];
              elseif strcmp(parms.Coh_time,'cluster')
                cohtix   = refpeak + [-clusterpad:clusterpad];
              elseif strcmp(parms.Coh_time,'psd')
                cohtix   = refpeak + [-clusterpad:clusterpad];
              else
                cohtix   = refpeak + [-clusterpad:clusterpad];
              end
            ...
            for ch   = 1:nchan
              ...
              if length(maxpeak)>1, maxpeak=maxpeak(1); end
            ...
            if psdtix(end) <= size(data.epochs.data,2)
              [Pxx,freq]  = periodogram(double(data.epochs.data(ref,psdtix)),[],parms.PSD_foi,round(Fs));
...
%           if parms.writelog,fprintf('Correlation coefficient between epoch mean coherence and PSD: %g (p=%g)\n',clusters(c).CohPSD_corrcoef,clusters(c).CohPSD_corrcoef_p);end
          if parms.plotflag
            if c == 1
              ...
              tmpavg = tmp; clear tmp
            else
              tmpavg.averages(c)      = tmpavg.averages(1);
              tmpavg.averages(c).data = clusters(c).avgdata;
            end
            tmpavg.averages(c).event_code = c;
            tmpavg.averages(c).num_trials = length(clusters(c).epochs);
            if c == length(peaktypes)
              ts_ezplot(tmpavg,'layout',layout,'title',sprintf('Cluster averages: ref=%s (%s)',reflabel,peaktype),'showlabels','yes','cond_labels',peaktypes);
              clear tmpavg
            end
          end
...
%   end
  ...
  if parms.plotflag
    ...
    for k = 1:length(SOreference)
      ...
      if k == 1, hfig1 = figure; else figure(hfig1); end
      ...
      for j = 1:nmax
        ...
        if mod(j+1,nr*nc)==1 && j+1<=nmax
          if k == 1, hfig2 = figure; else figure(hfig2); end
