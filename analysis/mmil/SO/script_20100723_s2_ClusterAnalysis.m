...
%   flipvec = pij(flipref,:)';
  ...
  fprintf(fid,'Saving file: %s\n',outfile);
  save(outfile,'events','peaks','flipvec','pij','rij','Ri','Eik','S','clusterstatus','InvolvedChans','badchans','tc');
  telapsed = toc(tstart); fprintf(fid,'Time elapsed: %g min\n',telapsed/60);
end
...
%   end
% % Reject based on involvement in too few trials
% tmp=arrayfun(@(x)length(x.cluster_indices),cluster_list(1).channel); 
% find(tmp~=0 & tmp<(mean(tmp)-std(tmp)))

  Fs      = flipdat.sfreq;
  bpfreq  = [10 100];
  foi     = [10:5:100];
  NCO     = linspace(3,8,length(foi)); width=NCO; %[]; Nco     = 7; % width in freqanalysis
  sf      = foi / width;
%   sf      = [2 2 3 5 5 5 5 5 5]; width = foi ./ sf;
  ...
  WaveletFile = strrep(ClusterFile,'cluster-structure',sprintf('ICA_TF_cluster_structure_epochs-%g-%gs',tcut,tcut));
...
%       clusters(c).parms.RiThreshold = TF_RiThreshold;
    ...
    save(WaveletFile,'clusters','-v7.3');
    telapsed = toc(tstart); fprintf(fid,'Time elapsed: %g min\n',telapsed/60);
    warning on
  end
tic  
...
tfz = SO_freqband_average(tfz,[8 12]);
ts_ezplot(TFR,'events',[1 2],'blc',1,'baselinetype','zscore','zlim',[-3 3],'verbose',0,'layout',layout,'chantype','grad');
...
load s2_ICA_TF_cluster_structure_epochs-0.5-0.5s_post-rejection_chan-then-trial_Ri0.3_20-Jul-2010.mat
tic
% preallocation    
for c = 1:length(clusters)
  for k = 1:length(clusters(c).epochs)
...
toc
  ...
  tfr2(1).timefreq.time = pad(sel)/Fs;
  tfr2(2).timefreq.time = pad(sel)/Fs;
  tfr2(1).timefreq.frequencies = foi;
  tfr2(2).timefreq.frequencies = foi;
...
blcwin = [-.35 -.15];%[tt(1) + [.3 .7]*(tt(end)-tt(1))]; 
zlim = [-3 3]; xlim = [-.4 .4]; foilim = [30 50];
tfz2 = ts_zscore(TFR2,'baselinetype','zscore','blcwindow',blcwin,'verbose',0);
tfz2 = SO_freqband_average(tfz2,foilim);
ts_ezplot(TFR2,'events',[1 2],'toilim',xlim,'blc',1,'baselinetype','zscore','blcwindow',blcwin,'zlim',zlim,'verbose',0,'layout',layout,'chantype','grad');
ts_ezplot(TFR2,'events',3,'toilim',xlim,'blc',1,'baselinetype','zscore','blcwindow',blcwin,'zlim',zlim,'verbose',0,'layout',layout,'chantype','grad','showlabels','yes');
ts_ezplot(tfz2,'events',[1 2],'toilim',xlim,'zlim',zlim,'layout',layout,'chantype','grad');
ts_ezplot(tfz2,'events',3,'toilim',xlim,'zlim',zlim,'layout',layout,'chantype','grad');


blcwin = 'all';%[-.35 -.15];%[tt(1) + [.3 .7]*(tt(end)-tt(1))]; 
zlim = [-3 3]; xlim = [-.8 .8]; foilim = [8 12];

% compare diff in (t<0)-(t>0) b/w up & down
TFR2   = ts_combine_data(tfr2);
tt     = TFR2.timefreq(1).time;
tfdiff = TFR2;
tfdiff.timefreq(1).power = TFR2.timefreq(1).power(:,tt<0,:) - flipdim(TFR2.timefreq(1).power(:,tt>0,:),2);
tfdiff.timefreq(2).power = TFR2.timefreq(2).power(:,tt<0,:) - flipdim(TFR2.timefreq(2).power(:,tt>0,:),2);
tfdiff.timefreq(1).time  = TFR2.timefreq(1).time(tt>0);
tfdiff.timefreq(2).time  = TFR2.timefreq(2).time(tt>0);
tfdiff = ts_combine_conditions(tfdiff,'combinations',{'1-2'},'neweventcodes',3);
ts_ezplot(tfdiff,'events',[1 2],'toilim',xlim,'blc',1,'baselinetype','zscore','blcwindow',blcwin,'zlim',zlim,'verbose',0,'layout',layout,'chantype','grad');
ts_ezplot(tfdiff,'events',3,'toilim',xlim,'blc',1,'baselinetype','zscore','blcwindow',blcwin,'zlim',zlim,'verbose',0,'layout',layout,'chantype','grad','showlabels','yes');
tfdz2 = ts_zscore(tfdiff,'baselinetype','zscore','blcwindow',blcwin,'verbose',0);
tfdz2 = SO_freqband_average(tfdz2,foilim);
ts_ezplot(tfdz2,'events',[1 2],'toilim',xlim,'zlim',zlim,'layout',layout,'chantype','grad');
ts_ezplot(tfdz2,'events',3,'toilim',xlim,'zlim',zlim,'layout',layout,'chantype','grad');
...
%     ts_ezplot(tmpTFR,'showlabels','yes','title',sprintf('cond%g, trial%g (t=%gs)',c,k,clusters(c).epochs(k).HistTime),'layout',layout,'chantype','grad','blc',1,'baselinetype','zscore','zlim',zlim,'verbose',0);
%     hfig(c) = gcf;
%     set(gcf,'Name',sprintf('t=%gs',clusters(c).epochs(k).HistTime));
%   end
%   pause
%   close(hfig);%close(1); close(2);
% end
...
for c = 1:length(clusters)
  ...
  for k = 1:N(c)
    tmpTFR = ts_data_selection(TFR,'chanlabel',clusters(c).epochs(k).InvolvedChans,'toilim',tlim,'foilim',flim);
    tmpTFR.timefreq = tmpTFR.timefreq(1);
    tmpTFR.timefreq.power = abs(double(clusters(c).epochs(k).TFR)).^2;
...
%     [tmpN,tmpX] = hist(chanpvals(ch,:),nbins);
...
figure;
channum      = 1:nchan;
channum      = channum([flipvec~=0]' & peaktimes(1,:)~=0 & peaktimes(2,:)~=0);
tmppeaktimes = peaktimes(:,[flipvec~=0]' & peaktimes(1,:)~=0 & peaktimes(2,:)~=0);
tmpvec       = diff(tmppeaktimes,[],1)./max(abs(tmppeaktimes),[],1);
subplot(2,1,1),try hist(tmppeaktimes(:),100); end; xlabel('peak power'); axis tight;% set(gca,'xlim',[min(chanX(:)) max(chanX(:))]);
subplot(2,1,2),plot(channum,tmpvec,'.'); xlabel('good chan #'); title('diff in power at t=0 bw pos & neg (normalized)'); axis tight; hline(0,'k');

figure
nbins = 1000;
tmp=pvals{1}(:);subplot(3,1,1),try hist(tmp(tmp~=0),nbins);   end; title(sprintf('cond 1 (n=%g)',sum(n1)));        xlabel('power'); ylabel('count');axis tight %axis([-2 4 0 20]);
tmp=pvals{2}(:);subplot(3,1,2),try hist(tmp(tmp~=0),nbins);   end; title(sprintf('cond 2 (n=%g)',sum(n2)));        xlabel('power'); axis tight %axis([-2 4 0 20]);
tmp=cat(1,pvals{:}); subplot(3,1,3),try hist(tmp(tmp~=0),nbins); end; title(sprintf('cond 1 & 2 (n=%g)',sum(n1)+sum(n2))); xlabel('power'); axis tight %axis([-2 4 0 40]);
set(gcf,'Name',sprintf('t=[%g %g], f=[%g %g]',tlim,flim));

figure
nbins = 400; ch = 9;
tmp=pvals{1}(:,ch);subplot(3,1,1),try hist(tmp(tmp~=0),nbins);   end; title(sprintf('cond 1 (n=%g)',sum(n1)));        xlabel('power'); ylabel('count');axis([-10 20 0 30]);
tmp=pvals{2}(:,ch);subplot(3,1,2),try hist(tmp(tmp~=0),nbins);   end; title(sprintf('cond 2 (n=%g)',sum(n2)));        xlabel('power'); axis([-10 20 0 30]);
tmp=cat(1,pvals{1}(:,ch),pvals{2}(:,ch)); subplot(3,1,3),try hist(tmp(tmp~=0),nbins); end; title(sprintf('cond 1 & 2 (n=%g)',sum(n1)+sum(n2))); xlabel('power'); axis([-10 20 0 60]);
set(gcf,'Name',sprintf('t=[%g %g], f=[%g %g]',tlim,flim));


% Power normalization
% calculate normalization curve
[bchans,jnk]  = match_str({flipdat.sensor_info.label},{peaks(find(flipvec==0)).label});
flipdat       = ts_data_selection(flipdat,'badchans',bchans);
nchan         = flipdat.num_sensors;
goodchans     = find(flipvec);
...
warning off,tic
fprintf('calculate normalization curve\n');
for c  = 1:length(clusters)
  ntrl  = length(clusters(c).epochs);    
  for k = 1:ntrl
...
toc
% apply normalization
fprintf('apply normalization curve\n');
clear alldat
for c  = 1:length(clusters)
  ntrl  = length(clusters(c).epochs);    
  for k = 1:ntrl
...
toc
fprintf('calculate normalized power\n');
warning on   
pow             = tfr(1);
...
for c = 1:length(clusters)
  ...
  for i  = 1:nchan
    % find clusters involving this channel
    cix  = find(arrayfun(@(x)(ismember(flipdat.sensor_info(i).label,x.InvolvedChans)),clusters(c).epochs));
    % get indices to the matrix in those clusters for this channel
    ind  = arrayfun(@(x)find(ismember(x.InvolvedChans,flipdat.sensor_info(i).label)),clusters(c).epochs(cix));
    % get data from those matrices for this channel
    tmp = cellfun(@(x,y)x(y,:,:),alldat(cix),num2cell(ind),'uniformoutput',false);
    % concatenate data along fourth dimension (trials)
    tmp = cat(4,tmp{:});
    % average POWER along fourth dimension (trials)
    pow(c).timefreq.power(i,:,:) = mean(tmp,4);
    clear tmp
  end
end
toc
pow = ts_combine_data(pow);
% average over trials of normalized power (normalization curve)
pow = ts_combine_conditions(pow,'combinations',{'1-2'},'neweventcodes',3);
powband = SO_freqband_average(pow,[30 100]);
ts_ezplot(pow,'zlim',[0 8],'events',[1 2],'verbose',0,'layout',layout,'chantype','grad');
ts_ezplot(pow,'zlim',[-1 1],'events',[3],'verbose',0,'layout',layout,'chantype','grad');
ts_ezplot(powband,'autoscale',1,'events',[1 2],'verbose',0,'layout',layout,'chantype','grad');
ts_ezplot(powband,'autoscale',1,'events',3,'verbose',0,'layout',layout,'chantype','grad');

% average over channels of normalized power (normalization curve)
figure
t = pow.timefreq(1).time; f = pow.timefreq(1).frequencies;
subplot(3,1,1),imagesc(t,f,squeeze(mean(pow.timefreq(1).power,1))'); title('cond 1'); caxis([1.4 2.4]); colorbar; vline(0,'k');
subplot(3,1,2),imagesc(t,f,squeeze(mean(pow.timefreq(2).power,1))'); title('cond 2'); caxis([1.4 2.4]); colorbar; vline(0,'k');
subplot(3,1,3),imagesc(t,f,squeeze(mean(pow.timefreq(3).power,1))'); title('difference'); caxis([-.12 .12]); colorbar; vline(0,'k');
...
clusters_orig = clusters;
tstart  = tic;
for c   = 1:length(clusters_orig)
  clusters = clusters_orig(c);
  ntrl  = length(clusters.epochs); 
  % loop over clusters
  for k = 1:ntrl
    fprintf('Type %g of %g: cluster %g of %g (%g min)\n',c,length(clusters_orig),k,ntrl,toc(tstart)/60);
    % select data in this cluster
    tk  = clusters.epochs(k).HistTime;
    ind = nearest(t,tk) + datasel;
    if ind(1) < 1, continue; end
    [sel1,sel2] = match_str({flipdat.sensor_info.label},clusters.epochs(k).InvolvedChans);
    ...
    clusters.epochs(k).TFR(:,:,:) = single(W.timefreq.cmplx);
    % clear vars
    clear y x W
  end
  save(sprintf('clusters_with_wavelet_spectra_cond%g.mat',c),'clusters','-v7.3');
end
toc(tstart)
clusters = clusters_orig;
...
%% S-PLV based on Hilbert Phase
clear data procdat seldat invpks allpks
load(ClusterFile);

bpfreq  = [8 14];     % sets the freq limits of the phase estimation
NCY     = 5;          % sets the size of the S-PLV integration window
...
% Define ROI comparisons (between and within ROIs)
comparisons = {[1 2]};%,[1 3],[3 4],[5 6],[1 1],[2 2],[3 3],[4 4]};

seltime = [flipdat.epochs.time(1) flipdat.epochs.time(end)];
foi     = mean(bpfreq,2);
FCPAD   = abs(diff(bpfreq,[],2)/2);
...
for c = 1:length(clusters)
  for k = 1:length(clusters(c).epochs)
    clusters(c).epochs(k).TFR = single(zeros(length(clusters(c).epochs(k).InvolvedChans),ntime,nfreq));
  end
end
...
for c   = 1:length(clusters_orig)
  ...
  for k = 1:ntrl
    ...
    for f = 1:nfreq
      ...
      % bandpass filter each freq
      xx  = ts_freq_filt(x',Fs,[f0-fcpad f0+fcpad],[0 0],'bandpass')';
      % calculate hilbert phaes
      phi = angle(hilbert(xx));
      y.epochs.data(:,:,f) = phi;
      clear xx
    end
    clusters.epochs(k).HilbertPhase(:,:,:) = single(y.epochs.data);
%     % wavelet analysis
%     W   = ts_freqanalysis_fieldtrip(y,'foi',foi,'sf',sf,'trials_flag',1,'save_flag',0,'verbose',0);
%     clusters.epochs(k).TFR(:,:,:) = single(W.timefreq.cmplx);
    % clear vars
  ...
  save(sprintf('clusters_with_HilbertPhase%g-%gHz_cond%g.mat',bpfreq,c),'clusters','-v7.3');
end
toc(tstart)

load(ClusterFile);

if ~exist('cluster_list','var')
  % make a list of cluster indices for each channel
  cluster_list = [];
  for ch = 1:nchan
    this = flipdat.sensor_info(ch).label;
...
for c = 1:2%length(clusters)
  clear clusters
  load(sprintf('clusters_with_HilbertPhase%g-%gHz_cond%g.mat',bpfreq,c),'clusters');
  cc = 1; % clusters index
  As = zeros(1,ntime,nfreq);              % straight average over ROI comparisons
  Aw = zeros(1,ntime,nfreq); An = [];     % weighted average over ROI comparisons (weight by ncombinations)
% %   condition(c).SPLV_cond_xROIavg = single([]);
  ...
  for s  = 1:length(comparisons)
    ...
    % loop over channel pairs
    for k = 1:ncmb
      fprintf('Condition %g of %g, comparison %g of %g, combination %g of %g (%g sec)\n',c,length(cluster_list),s,length(comparisons),k,ncmb,toc(tstart)/60);
      Cs = zeros(1,ntime,nfreq);                          % straight average over trials
      thiscmb = cmb(cmbindex(k),:);
      ch1     = thiscmb(1);
      ch2     = thiscmb(2);
...
%       % preallocate splv matrix for this pair
      ...
      for n = 1:ntrials
        ...
        res   = zeros(1,ntime,nfreq);
        % get spectra for those two channels & calc splv
        phi1  = double(clusters(cc).epochs(thistrial).HilbertPhase(ch1idx,:,:));
        phi2  = double(clusters(cc).epochs(thistrial).HilbertPhase(ch2idx,:,:));
        theta = phi1 - phi2;
        cmplx = exp(i*theta);
        % loop over freqs
        for f = 1:nfreq
  ...
  allconditions = condition;
  condition     = condition(c);
  save(sprintf('condition_with_HilbertPhase%g-%gHz_SPLV_cond%g.mat',bpfreq,c),'condition');
  condition     = allconditions;
  clear allconditions
end % end condition (c)

c=1; load(sprintf('condition_with_HilbertPhase%g-%gHz_SPLV_cond%g.mat',bpfreq,c)); allcond(c)=condition;
c=2; load(sprintf('condition_with_HilbertPhase%g-%gHz_SPLV_cond%g.mat',bpfreq,c)); allcond(c)=condition;
condition = allcond; clear allcond
tt        = [(-ntime/2):(ntime/2-1)]/Fs;

% plots
% compare condition(1 vs 2).SPLV_condition_average
figure
subplot(2,2,1),plot(tt,condition(1).SPLV_ROIs_average_straight); title('cond 1 (straight)'); set(gca,'xlim',[-.75 .75]); vline(0,'k');
subplot(2,2,2),plot(tt,condition(2).SPLV_ROIs_average_straight); title('cond 2 (straight)'); set(gca,'xlim',[-.75 .75]); vline(0,'k');
subplot(2,2,3),plot(tt,condition(1).SPLV_ROIs_average_weighted); title('cond 1 (weighted)'); set(gca,'xlim',[-.75 .75]); vline(0,'k'); xlabel('time');
subplot(2,2,4),plot(tt,condition(2).SPLV_ROIs_average_weighted); title('cond 2 (weighted)'); set(gca,'xlim',[-.75 .75]); vline(0,'k'); xlabel('time');

figure
plot(tt,condition(1).SPLV_ROIs_average_weighted,'b',tt,condition(2).SPLV_ROIs_average_weighted,'r');
set(gca,'xlim',[-.75 .75]); vline(0,'k'); legend('cond1','cond2'); title(sprintf('Hilbert Phase (%g-%gHz) S-PLV',bpfreq));
...
for c = 1:length(condition)
  for s = 1:length(condition(c).comparison)
    for k = 1:length(condition(c).comparison(s).channelpairs)
      pp = cat(1,pp,condition(c).comparison(s).channelpairs(k).SPLV_trial_average_straight);
      ll = {ll{:} sprintf('c%gs%gp%g',c,s,k)};
    end
  end
end
avg.sensor_info                       = avg.sensor_info(1);
[avg.sensor_info(1:size(pp,1))]       = deal(avg.sensor_info);
[avg.sensor_info(1:size(pp,1)).label] = deal(ll{:});
avg.num_sensors                       = size(pp,1);
