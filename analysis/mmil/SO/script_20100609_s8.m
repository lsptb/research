    ...
    for gtype  = 1:length(parms.chantype)
      ...
      for pktype = 1:length(parms.peaktype)
        ...
        for r = 1:length(REF)
          ...
          % peak detection (grad1 & grad2)
          if corrdetectionflag
            CorrSensorEventFile = sprintf('%s/%s_SOpeaks_filt%g-%gHz_toi%g-%g_%s_ref%s_smooth%gsec_zero2zero%g-%gsec_mindist%gsec_Ref-%s-flipped.mat',outpath,subjects{subj},parms.bpfreq,parms.toilim,'grad','all',parms.smooth_window,parms.zero2zero_limits,parms.mindist,reflabel);
