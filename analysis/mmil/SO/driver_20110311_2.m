...
ts_ezplot(ts_data_selection(seldata,'chantype','grad'),'layout',params.layout,'showlabels','yes');

% wavelet analysis on eeg, grad1, grad2, & norm
tic
foi     = 4:2:100; % Hz, frequencies of interest
sf      = 5;       % Hz, spectral resolution
tfdata  = ts_freqanalysis_fieldtrip(seldata,...
                                          'foi'         ,foi,...
                                          'sf'          ,sf,...
                                          'trials_flag' ,1,...
                                          'save_flag'   ,0,...
...
timefreq_data; clear tfdata;
save('s2_timefreq_data.mat','timefreq_data','-v7.3');

% z-score average gamma power for one event
evcode        = 2;
gamma_foi     = [4 100]; % [30 100], [20 100],[30 80]
blcwindow     = [-.7 -.4];
timefreq_data = ts_data_selection(tfdata,'events',evcode);
f             = timefreq_data.timefreq.frequencies;
gamma_fix     = find(f>=gamma_foi(1) & f<=gamma_foi(2));
...
% select random subset so that each channel has same number of trials
% ...use randsel{cond}{chan}
