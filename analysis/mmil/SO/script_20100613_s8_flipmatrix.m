...
% waveforms 
  ...
  if corrdetflag
    CorrSensorEventFile = sprintf('%s/%s_SO_flip_Ref%s-%s_%s.mat',outpath,subjects{subj},reflabel,refpeaktype,detectionstring);
    if exist('flip','var')
  ...
  % - select all cluster epochs containing >20% chans from all reference channels
  ...
  if parms.clusterflag
    ...
    ClusterSensorEventFile = sprintf('%s/%s_SO_cluster-flip-Ref%s-%s_peaks_%s.mat',outpath,subjects{subj},reflabel,refpeaktype,detectionstring);
...
%     end
%     evcode = [1]; visualizer(ts_data_selection(avgdat,'events',evcode));
