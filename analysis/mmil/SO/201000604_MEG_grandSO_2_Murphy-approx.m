...
tic
  ...
  suffix  = '_modified-Murphy-approx-3';
  ...
  parms                 = [];
  parms.fc1             = .5;     % Hz (lower cut-off freq prior to detection)
  parms.fc2             = 4;      % Hz (upper cut-off freq prior to detection)
  parms.monothresh      = -2;      % increase to be stricter; rec: stop at fc1
...
%     events = tmp;
    ...
    GroupEventFile = sprintf('%s/%s_SO_grand_events_%gsecSOepochs_maxsumlo%g_grad1_600-1350sec%s.mat',outpath,subjects{subj},tpad,ceil(maxsum_lowerlimit),suffix);
