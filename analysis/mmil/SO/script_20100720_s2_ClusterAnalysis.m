...
for c = 1:length(clusters)
  ...
  for k = 1:N(c)
    ...
    tmpTFR.timefreq = tmpTFR.timefreq(1);
    tmpTFR.timefreq.power = abs(double(clusters(c).epochs(k).TFR)).^2;
    tmpTFR = ts_zscore(tmpTFR,'baselinetype','zscore','verbose',0);
...
subplot(2,1,1),try hist(tmppeaktimes(:),100); end; xlabel('peak power'); set(gca,'xlim',[min(chanX(:)) max(chanX(:))]);
subplot(2,1,2),plot(channum,tmpvec,'.'); xlabel('good chan #'); title('diff in power at t=0 bw pos & neg (normalized)');

figure
nbins = 300;
subplot(3,1,1),try hist(pvals{1},nbins);   end; title(sprintf('cond 1 (n=%g)',n1));        xlabel('power'); ylabel('count'); axis([-2 4 0 20]);
subplot(3,1,2),try hist(pvals{2},nbins);   end; title(sprintf('cond 2 (n=%g)',n2));        xlabel('power'); axis([-2 4 0 20]);
subplot(3,1,3),try hist([pvals{:}],nbins); end; title(sprintf('cond 1 & 2 (n=%g)',n1+n2)); xlabel('power'); axis([-2 4 0 40]);
