...
%  last mod: 07/19/10   by Don Hagler
%
...
parms = mmil_args2parms(varargin, { ...
  ...
  'write_log_flag',false,[false true],...  %SK 
  'rootoutdir',pwd,[],... %SK
  'prefix','proc',[],...  %SK
...
if parms.save_epochs_flag
  for j=1:length(event_codes),
    ...
    output_data.epochs(j).trial_info = {}; %AS 11/29 %SK
        output_data.epochs(j).trial_info.number=[];
        output_data.epochs(j).trial_info.latency=[];
        output_data.epochs(j).trial_info.badtrial=[];
        output_data.epochs(j).trial_info.event_code=[];
        output_data.epochs(j).trial_info.epoch_num=[];
        output_data.epochs(j).trial_info.duration=[];
        output_data.epochs(j).trial_info.datafile={};
        output_data.epochs(j).trial_info.events_fname={};
  ...
  for j=1:length(event_codes),
    ...
    output_data.averages(j).trial_info = {};%AS 11/29  %SK
        output_data.averages(j).trial_info = {}; %AS 11/29 %SK
        output_data.averages(j).trial_info.number=[];
        output_data.averages(j).trial_info.latency=[];
        output_data.averages(j).trial_info.badtrial=[];
        output_data.averages(j).trial_info.event_code=[];
        output_data.averages(j).trial_info.epoch_num=[];
        output_data.averages(j).trial_info.duration=[];
        output_data.averages(j).trial_info.datafile={};
        output_data.averages(j).trial_info.events_fname={};
...
if parms.write_log_flag
  [pathstr, name, ext, versn] = fileparts(datafile);
  logfile=sprintf('%s/matfiles/%s_avg.log',...
        parms.rootoutdir,name);
...
while evnum < length(parms.evnts)
  ...
  if(read_endsamp >= parms.hdr.tlast*parms.hdr.sfreq)
    ...
    if parms.write_log_flag %SK
        fprintf(fid, '%i\t %i\t %i\t %s\t %s\n', ...
          [], parms.evnts(evnum).condition, ...
          parms.evnts(evnum).latency, 'reject',  sprintf('reached end of file'));
        fprintf('%s: Rejected epoch triggered at %i (Event code %i)  <- reached end of file \n',mfilename, ...
          parms.evnts(evnum).latency, parms.evnts(evnum).condition);
    end;
    break;
  end;
  ...
  for tmp_evnum=1:length(tmp_evnts)
    ...
    if parms.save_epochs_flag && output_data.epochs(j).num_trials>=parms.max_num_trials
      break;
    elseif ~parms.save_epochs_flag && output_data.averages(j).num_trials>=parms.max_num_trials
      break;
    end;
    ...
    % classify rejects
    % SK fix to prevent skipping good trials in a tmp batch
    ...
    if reject_event_flag
      if parms.write_log_flag
        fprintf(fid, '%i\t %i\t %i\t %s\t %s\n', ...
          [], tmp_evnts(tmp_evnum).condition, ...
          tmp_evnts(tmp_evnum).latency, 'reject',  sprintf('Channel %s above threshold',parms.hdr.sensors.label{k}));
        fprintf('%s: Rejected epoch triggered at %i (Event code %i)  <- Channel %s above threshold \n',mfilename, ...
          tmp_evnts(tmp_evnum).latency, tmp_evnts(tmp_evnum).condition, parms.hdr.sensors.label{k});
      end
      if parms.save_epochs_flag
        switch parms.hdr.sensors.typestring{k}
        case 'mag'
    ...
    %%    
%         output_data.epochs(j).trial_info.trial_number(end+1) = evnum+tmp_evnum-1; %AS changed to trial number instead of number %SK perhaps we should not collect info for badtrials at all, just print them out in the log
%         output_data.epochs(j).trial_info.latency(end+1) = parms.evnts(evnum+tmp_evnum-1).latency; %SK
%         output_data.epochs(j).trial_info.badtrial(end+1) = 1; %SK
%         output_data.epochs(j).trial_info.epoch_num(end+1) = 0;  %SK
%         output_data.epochs(j).trial_info.duration(end+1) = 1; %AS 
%         output_data.epochs(j).trial_info.datafile{end+1} = datafile{:}; %AS I believe this is the data file 
%         if(isfield(parms.evnts,'fname'))
%             output_data.epochs(j).trial_info.events_fnames{end+1} =  parms.evnts(evnum+tmp_evnum-1).fname; %AS added fname to evnts data structure in ts_import_events
%         end;
...
%         output_data.averages(j).trial_info(end+1).number = evnum+tmp_evnum-1; %SK
%         output_data.averages(j).trial_info(end).latency = parms.evnts(evnum+tmp_evnum-1).latency; %SK
%         output_data.averages(j).trial_info(end).badtrial = 1; %SK
%         output_data.averages(j).trial_info(end).epoch_num = 0;  %SK
        %%
%         output_data.averages(j).trial_info.trial_number(end+1) = evnum+tmp_evnum-1; %AS changed to trial number instead of number %SK perhaps we should not collect info for badtrials at all, just print them out in the log
%         output_data.averages(j).trial_info.latency(end+1) = parms.evnts(evnum+tmp_evnum-1).latency; %SK
%         output_data.averages(j).trial_info.badtrial(end+1) = 1; %SK
%         output_data.averages(j).trial_info.epoch_num(end+1) = 0;  %SK
%         output_data.averages(j).trial_info.duration(end+1) = 1; %AS 
%         output_data.averages(j).trial_info.datafile{end+1} = datafile; %AS I believe this is the data file
%         if(isfield(parms.evnts,'fname'))
%             output_data.averages(j).trial_info.events_fnames{end+1} =  parms.evnts(evnum+tmp_evnum-1).fname; %AS added fname to evnts data structure in ts_import_events
%         end;
       end;
     end;
%%
    if ~reject_event_flag % add to data matrices only if not rejected
      % add event to trial_info %SK begin trial_info
    if parms.save_epochs_flag
%        output_data.epochs(j).trial_info(end+1).number = evnum+tmp_evnum-1;
%         output_data.epochs(j).trial_info(end).latency  = parms.evnts(evnum+tmp_evnum-1).latency;
%         output_data.epochs(j).trial_info(end).badtrial = 0;
    output_data.epochs(j).trial_info.trial_number(end+1) = evnum+tmp_evnum-1; %AS changed to trial number instead of number %SK perhaps we should not collect info for badtrials at all, just print them out in the log
    output_data.epochs(j).trial_info.latency(end+1) = parms.evnts(evnum+tmp_evnum-1).latency; %SK
    output_data.epochs(j).trial_info.badtrial(end+1) = 0; %SK
    output_data.epochs(j).trial_info.epoch_num(end+1) = 0;  %SK
    if parms.events(evnum+tmp_evnum-1).duration ~=0
        output_data.epochs(j).trial_info.duration(end+1) = parms.events(evnum+tmp_evnum-1).duration;
    else
        output_data.epochs(j).trial_info.duration(end+1) = 1; %AS
    end;
    ...
    output_data.epochs(j).trial_info.event_code(end+1) = parms.evnts(evnum+tmp_evnum-1).condition;
    
    
        if isfield(output_data.epochs(j).trial_info,'epoch_num')
            output_data.epochs(j).trial_info.epoch_num(end+1) = max([output_data.epochs(j).trial_info.epoch_num]) + 1;
        else
            output_data.epochs(j).trial_info.epoch_num(end+1) = 1;
        end
    else
        output_data.averages(j).trial_info.number(end+1) = evnum+tmp_evnum-1;
        output_data.averages(j).trial_info.latency(end+1)  = parms.evnts(evnum+tmp_evnum-1).latency;
        output_data.averages(j).trial_info.badtrial(end+1) = 0; %Changed to 1 
%         if isfield(output_data.averages(j).trial_info,'epoch_num')
%             output_data.averages(j).trial_info.epoch_num(end+1) = max([output_data.averages(j).trial_info.epoch_num]) + 1;
%         else
            output_data.averages(j).trial_info.epoch_num (end+1)= 1;
%         end
        if parms.evnts(evnum+tmp_evnum-1).duration ~=0
            output_data.averages(j).trial_info.duration(end+1) = parms.evnts(evnum+tmp_evnum-1).duration;
        else    
            output_data.averages(j).trial_info.duration(end+1) = 1;
        end; 
        output_data.averages(j).trial_info.datafile{end+1} = datafile;
        if(isfield(parms.evnts,'fname'))
            output_data.averages(j).trial_info.events_fnames{end+1} =  parms.evnts(evnum+tmp_evnum-1).fname;
        end;
       output_data.averages(j).trial_info.event_code(end+1) = parms.evnts(evnum+tmp_evnum-1).condition;
    end
    % SK trial_info end
    %AS trial_info
    ...
    if parms.save_epochs_flag
      ...
      num_trials = output_data.epochs(j).num_trials;
    else
      ...
      num_trials = output_data.averages(j).num_trials;
    end;
    ...
    if parms.write_log_flag
        fprintf(fid, '%i\t %i\t %i\t %s\t %s\n', num_trials, tmp_evnts(tmp_evnum).condition, ...
            tmp_evnts(tmp_evnum).latency, 'ok',  '');
    end;
    
    end;
  end;
   if length(tmp_evnts)>1 % more than one event in tmp_evnts %SK
      % add to evnum to keep track that we are processing next event not already processed
      evnum = evnum+length(tmp_evnts)-1; %SK
   end;
end
