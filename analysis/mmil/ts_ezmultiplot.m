...
elseif isempty(parms.conditions)
  ...
  parms.conditions             = sort(parms.conditions);
end
...
%     und,'Position',[1 1 figwidth figwidth]);
  ...
  chanX       = lay(p).pos(sellay, 1); cx = max(chanX) - min(chanX) + 2;
  chanY       = lay(p).pos(sellay, 2); cy = max(chanY) - min(chanY) + 2;
...
%       end
    ...
    chanHeight = chanWidth * ch;
    % check for zero
    if all(chanWidth==0) || all(chanHeight==0)
      chanX       = lay(p).pos(sellay, 1);
      chanY       = lay(p).pos(sellay, 2);
      chanWidth   = lay(p).width(sellay);  
      chanHeight  = lay(p).height(sellay);
    end
  elseif any(chanX(:)<0) || any(chanY(:)<0)
    scale_fact = .95;
    Xmin = min(chanX(:)); Ymin = min(chanY(:));
    Xmax = max(chanX(:)); Ymax = max(chanY(:));
    chanHeight = chanHeight / (Ymax-Ymin);
    chanWidth = chanWidth / (Xmax-Xmin);
    chanX = chanWidth(1)/10 + (1-chanWidth(1))*((chanX-Xmin)/(Xmax-Xmin));
    chanY = (1-chanHeight(1))*((chanY-Ymin)/(Ymax-Ymin));
    chanHeight = scale_fact*chanHeight;
    chanWidth = scale_fact*chanWidth;
    
%     chanWidth  = scale_fact * chanWidth / (max(chanX)-min(chanX));
%     chanHeight = scale_fact * chanHeight / (max(chanY)-min(chanY));
%     chanX      = .02 + scale_fact * (chanX - min(chanX)) / (max(chanX)-min(chanX));
%     chanY      = .02 + scale_fact * (chanY - min(chanY))/  (max(chanY)-min(chanY));
  end
...
%       end
    ...
    if all(isfinite(plotpos))
      subplot('Position',[chanX(k) chanY(k) chanWidth(k) chanHeight(k)],'nextplot','add')
      hold on
    else
...
%%
      ...
      if tfr_flag
        ...
        if parms.trials_flag
          ...
          plot(x,nanmean(y,2),'k-','linewidth',2,'Clipping','off')
        else
          plot(x,y,parms.graphcolor(c),'linestyle',parms.linestyle,'linewidth',parms.linewidth,'Clipping','off')
          if parms.showerror_flag
            if ~isfield(data.(datafield),'stdev')
              warning('Need standard deviation to plot standard error.');
            elseif data.(datafield)(c).num_trials <= 1
              warning('Need more than one trial to plot standard error.');
            else
              se  = data.(datafield)(c).stdev(seldat(k),xidc) / sqrt(data.(datafield)(c).num_trials);
              z1  = y + se;
              z2  = y - se;
              X   = [x  fliplr(x) ]';
              Y   = [z1 fliplr(z2)]';
              if ~isreal(Y)
                fprintf('Not showing standard error. Your stdev contains an imaginary part!\n');
              else
%                 try
                  pa1 = patch(X,Y,parms.graphcolor(c),'FaceAlpha',parms.transparency*1.5,'LineStyle','none','Clipping','off');
%                 catch
%                   pa1 = patch(X,Y,parms.graphcolor(c),'LineStyle','none','Clipping','off');
%                 end
      ...
      if mask_flag && ~tfr_flag && size(maskvector,1) >= k
        ...
        for kk = 1:length(edges)/2
          ...
          fill([ix1 ix1 ix2 ix2],[iy1 iy2 iy2 iy1],parms.statcolor,'EdgeColor','k',...
            'FaceAlpha',parms.transparency,'EdgeAlpha',0);%parms.transparency);
        end                      
      end
      % baseline stats
      if mask_flag && ~tfr_flag && iscell(parms.baseline_mask) && numel(parms.baseline_mask)==length(parms.conditions)
        sigmask = squeeze(parms.baseline_mask{c}(seldat(k),xidc,:) > 0);
        edges=[]; 
        offset = (c)*(ymax-ymin)/30;
        if ~isempty(sigmask),
            ylims=get(gca,'YLim');
            xlims=get(gca,'XLim');
            ...
            if sum(ons) > 0,
                for n = 1:length(ons),
                    line([x(ons(n)) x(offs(n))],[ymax ymax]-offset,'Color',parms.graphcolor(c),'LineWidth',1.5);%2*parms.linewidth);
                end
            end
            edges=[ons; offs];
        end        
      ...
      if c == length(parms.conditions)
        ...
        if strcmp(parms.showlabels,'yes')
          title(strrep(chanLabels{k},'_','\_'),'FontSize',parms.fontsize,'VerticalAlignment','baseline','Clipping','off')
        end
        % Box
        ...
        if ~strcmp(parms.axes,'no') || strcmp(parms.zerolines,'yes')
          ...
          if strcmp(parms.axes,'no')
            ...
            if tfr_flag
              ylims = get(gca,'ylim');
              if all(ismember(y,[2:12 14:2:24 25:5:55 70:10:200]))
                yvals = [2 14 25 55*(ymax<70)+70*(ymax>=70) ymax];
                ntick = length(yvals);
                ylocs = unique(round(linspace(ylims(1),ylims(2),ntick)));
              else
                ntick = min(parms.maxtickmarks,length(yticks));
                ylocs = unique(round(linspace(ylims(1),ylims(2),ntick)));
                if length(ylocs)~=ntick, ntick=length(ylocs); end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  if ~strcmpi(parms.comment,'none')
    ...
    for c = 1:length(parms.conditions)
      condstr = strrep(parms.cond_labels{c},'\','');
      condstr = strrep(condstr,'_','\_');
      annotation('textbox','Position',[.9 .85-(c-1)*.05 .1 .1],'VerticalAlignment','top',...
                 ...
                 'String',sprintf('%s\nNum Trials: %g',condstr,data.(datafield)(c).num_trials));
    end
