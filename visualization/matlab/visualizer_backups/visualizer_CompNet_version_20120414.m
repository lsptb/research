...
%   end    
  ...
  if get(gethandle('topoplotchk'),'value')
    %     ndim = 3; % # phase space dimensions
    %     npart = 8; % # partitions
    %     delay = .1; % sec
    % topo edit box => delay
    % sti threshold => npart
    % num clusters => ndim
    delay = str2num(get(gethandle('nframes'),'string'));
    npart = str2num(get(gethandle('rejsti') ,'string'));
    ndim = str2num(get(gethandle('clustercnt'),'string'));
    tau = max(1,round(delay*UserData.(type).hdr.sfreq));
    
    fprintf('Calculating entropy (t=%g-%gsec,ndim=%g,npart=%g,tau=%gsec (%g samples))\n',xlim,ndim,npart,delay,tau);
    for j = 1:length(alldata)
      for k = 1:length(uniqsens)
        X = alldata{j}(k,:);
        hT = getEntropy(X,ndim,npart,tau);
        fprintf('j=%g: %s: Topological Entropy: %g\n',j,uniqsens{k},hT);        
      end
    end
...
function hT = getEntropy(x,ndim,npart,tau)
% declare parameters
%ndim = cfg.ndim; % 4 , # dimensions in (embedding) phase space
%npart = cfg.npart; % 8, # partitions
%tau = cfg.tau; % # steps in delay = round([step in sec]*Fs)
...
mx = max(x);
mx = mx + 1e-6*(mx-mn);
bnm1 = dataToBoxNo(x,1,mn,mx,npart,ndim,tau);
err = 0;
for i = 2:(nsamp-(ndim-1)*tau)
  bn = dataToBoxNo(x,i,mn,mx,npart,ndim,tau);
  if isnan(bnm1) || isnan(bn)
    err = 1;
    break;
  end
  hT(bnm1,bn) = 1;
...
if err
  ...
  hT = log(max(abs(eig(hT))));
end

function BoxNo = dataToBoxNo(x,i,mn,mx,N,D,tau)
b = zeros(1,D);
for j = 0:D-1
  pnt = x(i+j*tau);
  b(j+1) = floor(N*(pnt-mn)/(mx-mn))*N^j;
end
BoxNo = sum(b)+1;
