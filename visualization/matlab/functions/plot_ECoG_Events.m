...
%   set(gca,'YTickLabel',labels(I));  
  ...
  if ~isempty(cst)
    ...
    colorbar; title('Delay map, involves 1 grid only'); % axis square;
    subplot(2,1,2);
  ...
  for layoutnum = 1:length(g)
    ...
    ts_ezplot(ts_data_selection(tmp,'channels',g{layoutnum}),'showlabels','yes','title',sprintf('grid %g (trial %g), REF = %s',layoutnum,trl,params.markers(k)),'newfig',0,'layout',lay{layoutnum});%,'vline',0); % cst.epochs(trl).RefChan)
  end
