...
%       end
      ...
      if maskflag
        ...
        [dat,parms] = apply_mask(dat,parms,[dat.(datafield).event_code]);  %%%%%%%%% ans = 1, 2 etc 6
        [jnk cix] = intersect({sensor_orig.label},{dat.sensor_info.label});  %%%%% arive here with mask = []
...
if ~isempty(parms.statfile) 
  ...
  load(parms.statfile);
elseif ~isempty(parms.stat_data)  %%%%%%%%%%%%  Not Empty
  stat_data = parms.stat_data;  %%%%%%%%%%% but contains no mask field
else
  return;
end

if length(evcode) == 2  %%%%%%%%%%%%%%%% false - SRD
%%%%%%%%%%%%%%%%%%%%%%%  This should be '>= 2' with additions for the rest of the events below.
%%%%%%%%%%%%%%%%%%%%%%%  This file with debug comments was accidently saved over Jasons due to rwx file protect.
%%%%%%%%%%%%%%%%%%%%%%%  SRD svn copy will replace this in next release.
...
elseif length(evcode) == 1 && ismember(evcode,[stat_data.stats.event_code])   %%%%%%%%%%%%%%% false
  mask = stat_data.stats([stat_data.stats.event_code]==evcode).prob <= parms.alpha;
elseif length(evcode) == 1  %%%%%%%%%% false
  mask = stat_data.stats(1).prob <= parms.alpha;
else
  return;  %%%%%%%%%%%%%% it does with baseline_mask = [], mask =[], maskstyle transparency, statfile=[], maskflag=1, evcode [1 2 3 4 5 6]
end
...
if isnumeric(opt.colorbar) && ~isempty(opt.colorbar)
  if     opt.colorbar == 1,  parms.colorbar = 'yes';
  elseif opt.colorbar == 0,  parms.colorbar = 'no'; end
end
