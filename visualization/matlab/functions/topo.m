function handles_plot = topo(data,varargin)
...
if isempty(cfg.fig)
  ...
  figure('NumberTitle','on','Color','w','Position',[1 1 .9*screensize(3) .9*screensize(4)],'nextplot','add');
  hold on
...
xstepsize = 1 / (cfg.ncols+xoffset);
ystepsize = 1 / (cfg.nrows+yoffset);
xstep = mod((1:cfg.ncols+xoffset)-1,cfg.ncols);
ystep = mod((1:cfg.nrows+yoffset)-1,cfg.nrows)+1;
...
for r = 1:cfg.nrows           % row index
  for c = 1:cfg.ncols         % column index
    cnt = cnt + 1;
    if ~isempty(cfg.parent) && ~isequal(cfg.parent,0)
%       handles.subplot(cnt) = subplot('position',[xpos(cnt) ypos(cnt) xstepsize ystepsize],'parent',cfg.parent,'units','normalized','nextplot','add');
      subplot('position',[xpos(cnt) ypos(cnt) xstepsize ystepsize],'parent',cfg.parent,'units','normalized','nextplot','add');
    elseif ~isequal(cfg.parent,0)
      subplot(cfg.nrows,cfg.ncols,k,'nextplot','add')
    end
    hold on
    ...
    tt   = t(samp);
    tlim = [tt(1) tt(end)];
    cfg.comment = sprintf('t=[%10.10g %10.10g]',tlim);
    ...
    handles_plot(cnt) = draw_topoplot(Xi,Yi,Zi,x,y,delta,cfg,dat);
    hold off
    axis tight;%square
    k = k + 1;
...
function h = draw_topoplot(Xi,Yi,Zi,x,y,delta,cfg,dat)
if ~cfg.iEEG_flag
...
%     hp2 = plot(y(cfg.normal),    x(cfg.normal),    cfg.emarker,  'Color', cfg.ecolor,  'markersize', cfg.emarkersize);
    tmp = (dat/max(abs(dat(:))));
    tmp1 = .01+cfg.emarkersize*(tmp-min(tmp(:)));
    tmp2 = .01+(tmp-min(tmp(:)));
    hp2 = scatter(y(cfg.normal),    x(cfg.normal),tmp1*(.01*numel(Zi)),2*tmp2*prctile(Zi(:),70),'.');
  end
