...
handles.topochk = uicontrol('style','checkbox','string','topo','value',0,'parent',handles.pcontrol,'units','normalized','position',[.4 .05 .05 .45],'callback',{@update,'topo','wave'});
  
handles.properties.nplot = size(y,1);
for k = 1:size(y,1)
  handles.subplot(k) = subplot('position',[xpos(k) ypos(k) xsiz(k) ysiz(k)],'units','normalized','parent',handles.pview);
...
if strcmp(type,'wave'); %1 % plot waveform
  ...
  if get(handles.topochk,'value')==1
    ...
    handles.topoaxes = axes('Position',[0 0 1 1],'units','normalized','visible','on','parent',handles.pview);
    axes(handles.topoaxes);
    tmpdat = alldata;
    tmpdat.epochs.data = alldata.epochs.data(:,I);
    tmpdat.epochs.time = alldata.epochs.time(I);
    topo(tmpdat,'avgovertime','yes','zlim',handles.properties.clim,'nrows',1,'ncols',1,...
        'layout',handles.layout.file,'iEEG_flag',1,'showlabels','yes','fig',0); 
    pause(.1);
  else
%     if strcmp(get(handles.subplot(1),'visible'),'off')
