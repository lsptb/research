function [yo,fo,to] = specmw(varargin)
...
if nargout == 0
    ...
    if length(t)==1
        imagesc([0 1/f(2)],f,(abs(y)+eps).^2); axis xy; ylim([0 100]); colormap(jet), colorbar %caxis([0 15]);
    else
        imagesc(t,f,(abs(y)+eps).^2); axis xy; ylim([0 100]); colormap(jet), colorbar %caxis([10 200.52000]);
    end
    xlabel('Time')
    ylabel('Frequency')
    title('Spectrogram')
