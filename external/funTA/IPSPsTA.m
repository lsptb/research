function O = IPSPsTA(x,yF,yIC,Size,Notch,method,FreqRange,Bins,SmoothWindow,PlotsYN)
...
dF = yF.*1e6;
if ~isempty(Notch), dF = Bsfft(dF,Fs,Notch(1),Notch(2)); end
...
if strcmp('Y',PlotsYN)
    ...
    if t_factor>1, dIC_orig = downsample(dIC_orig,t_factor); dF = downsample(dF,t_factor); t = downsample(t,t_factor); Fs = Fs/t_factor; end
...
[st,sti] = findpeaks(dIC);
[tr,tri] = findpeaks(dIC*-1);
tr = tr.*-1;
...
for k = 1:length(STi)-2
    try
        wetemp = crossing(dIC(TRi(k):end),[],SThh(k));
    catch ME1
    ...
    if ~isempty(wetemp)
        ...
        wstarti(k) = length(dIC) - wstarti(k);
...
dFA = dFA - mean(dFA);
for k = 1:length(STi3)
    if round(0.005*Fs)<STi3(k) && STi3(k)<(length(dIC)-round(0.15*Fs))
...
for k = 1:length(STi)
    if round(0.01*Fs)<STi(k) && STi(k)<(length(dIC)-round(0.15*Fs))
...
if strcmp('Y',PlotsYN)
    ...
    plot(t,dIC_orig,'-c'), plot(t,dIC,'-k')
...
maxIC = max(dIC(round(ct(1)*Fs):round(ct(2)*Fs)));
minIC = min(dIC(round(ct(1)*Fs):round(ct(2)*Fs)));

%% cross correlations
[co,lags] = xcov(dF(round(ct(1)*Fs):round(ct(2)*Fs)),dIC(round(ct(1)*Fs):round(ct(2)*Fs)));
[coAIC] = xcov(dIC(round(ct(1)*Fs):round(ct(2)*Fs)));
...
if strcmp('Y',PlotsYN)
    ...
    subplot(3,3,[4 5]), hold on, plot(t,dIC_orig,'-k'), plot(t,dIC,':b'), axis tight, xlim(ct), ylim([minIC maxIC])
...
if strcmp('Y',PlotsYN)
    ...
    while P==0
        ...
        if ~isempty(ans3{1})
            ...
            dF2 = dF2 - mean(dF2);
            for k = 1:length(STi3)
                if round(0.004*Fs)<STi3(k) && STi3(k)<(length(dIC)-round(0.15*Fs))
