...
function explorestructTA(varargin)
...
haxes = axes('Units', 'normalized','Position', [.52 .182 .434 .653], 'Box', 'on', 'XTick', [], 'YTick', []);
%haxes2 = axes('Units', 'normalized','Position', [.873 .055 .06 .05],  'XTick', [], 'YTick', []);
%logo = load ('Stbrowser');
%image(logo.A, 'Parent', haxes2);
%set(haxes2,  'visible','off');
pb1a = uicontrol('String', 'plot', 'Units', 'normalized',...
                        'Position', [.357 .64 .035 .083] , 'callback', @plotselected_cba,...
                        'TooltipString', 'plot');
pb1b = uicontrol('String', 'image', 'Units', 'normalized',...
                        'Position', [.391 .64 .035 .083] , 'callback', @plotselected_cbb,...
                        'TooltipString', 'imagesc');
pb1c = uicontrol('String', 'bar', 'Units', 'normalized',...
                        'Position', [.426 .64 .035 .083] , 'callback', @plotselected_cbc,...
                        'TooltipString', 'bar');
pb1d = uicontrol('String', 'hist', 'Units', 'normalized',...
                        'Position', [.46 .64 .035 .083] , 'callback', @plotselected_cbd,...
                        'TooltipString', 'hist');
                    
pb2 = uicontrol('String', 'Disp Selected', 'Units', 'normalized',...
                        'Position', [.385 .452 .086 .083], 'callback', @displayselected_cb,...
                        'TooltipString', ' Display Selection in command ');
pb3 = uicontrol('String', 'Export', 'Units', 'normalized',...
                        'Position', [.385 .35 .086 .083], 'callback', @exportselected_cb,...
                        'TooltipString', ' Export Selection to workspace ');
pb3b = uicontrol('String', 'Export Multiples', 'Units', 'normalized',...
                        'Position', [.385 .240 .086 .083], 'callback', @exportselected_cb2,...
                        'TooltipString', ' Export Selection to workspace ');
                    
txt1 = uicontrol('String', '', 'Units', 'normalized', 'Style', 'Edit',...
                            'Position', [.52 .915 .145 .034], 'BackgroundColor',  [.925 .914 .847]);
txt2 = uicontrol('String', '', 'Units', 'normalized', 'Style', 'Edit',...
                            'Position', [.665 .915 .145 .034], 'BackgroundColor',  [.925 .914 .847]);
txt3 = uicontrol('String', '', 'Units', 'normalized', 'Style', 'Edit',...
                            'Position', [.81 .915 .145 .034], 'BackgroundColor',  [.925 .914 .847]);
...
col1 = uicontrol('String', 'Name', 'Units', 'normalized', 'Style', 'Text',...
                            'Position', [.52 .949 .145 .03], 'BackgroundColor',  [.925 .914 .847]);
col2 = uicontrol('String', 'Size', 'Units', 'normalized', 'Style', 'Text',...
                            'Position', [.665 .949 .145 .03], 'BackgroundColor',  [.925 .914 .847]);
col3 = uicontrol('String', 'Class', 'Units', 'normalized', 'Style', 'Text',...
                            'Position', [.81 .949 .145 .03], 'BackgroundColor',  [.925 .914 .847]);
label = uicontrol('String', 'Selected''s info', 'Units', 'normalized', 'Style', 'Text',...
                            'Position', [.404 .91 .1 .034], 'BackgroundColor',  [.925 .914 .847]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plotselected_cba(h, ev)
        tmp =  tree. FigureComponent;
        S = get(tmp, 'UserData');
        s = S{1};
        cNode = S{2};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plotselected_cbb(h, ev)
        tmp =  tree. FigureComponent;
        S = get(tmp, 'UserData');
        s = S{1};
        cNode = S{2};
        ...
        if isnumeric(val) & length(val)>1
                    if isreal(val)
                        imagesc(val,'Parent',haxes), axis tight, axis xy
                        title(haxes, plotted)
                    else
                        imagesc(abs(val),'Parent',haxes), axis tight, axis xy
                        title(haxes, plotted)
                    end
        else
            errordlg('variable to be plotted should be numeric with length > 1')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plotselected_cbc(h, ev)
        tmp =  tree. FigureComponent;
        S = get(tmp, 'UserData');
        s = S{1};
        cNode = S{2};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plotselected_cbd(h, ev)
        tmp =  tree. FigureComponent;
        S = get(tmp, 'UserData');
        s = S{1};
        cNode = S{2};
        ...
        if isnumeric(val) & length(val)>1
                    if isreal(val)
                        bins = linspace(min(min(val)),max(max(val)),25);
                        bar(haxes,bins,histc(val,bins),'BarWidth',1), shading interp
                        set(haxes, 'XLim', [bins(1) bins(end)])
                        title(haxes, plotted)
                    else
                        bar(haxes,abs(val))
                        set(haxes, 'XLim', [0 length(val)+1])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function exportselected_cb2(h, ev)
        %Exporting data from a tree to the MATLAB workspace 
        tmp =  tree. FigureComponent;
        S = get(tmp, 'UserData');
        s = S{1};
        ...
        s1 = inputdlg({'Structure hierarchy up to multiples:';'Structure hierarchy from multiples to desired output:'},'',1,{'Results.function' 'output_1'});
        fieldname = cNode.getValue;
        temp1 = evalin('caller',s1{1});
        temp2 = strsplit(s1{2},' ');
        [val,excluded] = StructF2Mat(temp1,fieldname,temp2);
        assignin('base', 'V', val);
        assignin('base', 'excluded', excluded);
        disp(strcat(fieldname, ' is assigned to ''V'' in workspace, with excluded multiples in ''excluded'''))
